...
export default function ConfirmarSolicitudPage() {
  ...
  useEffect(() => {
    ...
    const solicitud = JSON.parse(s);

    // AGREGAR MONTO SI NO EXISTE
    if (!solicitud.monto) {
      solicitud.monto = 50; // monto base
    }

    setResumen(solicitud);
    ...
    try {
      ...
      const response = await fetch("/api/solicitudes/crear", {
        ...
        body: JSON.stringify({
          ...
          monto: resumen.monto, // ahora SIEMPRE existe
          metodo_pago: null,
        }),
      });
  ...
  return (
    <main className="max-w-2xl mx-auto p-8">
      ...
      <div className="bg-white rounded-lg shadow p-6 mb-6">
        ...
        <div className="my-3">
          <b>Personal seleccionado:</b>{" "}
          {personal
            ? `${personal.nombre} ${personal.apellido}`
            : "Por asignar (Cualquiera)"}
        </div>
        {personal?.foto_url && (
          <img
